...
- Signing out from the user menu lands on the sign-in page instead of leaving the visitor on the page they were just reading, which may be one they no longer have access to. Only on success — a failed sign-out keeps its error and stays put — and only for that menu: the checkout's two sign-outs are deliberate, one continues as a guest on the same page and the other already goes to the sign-in page itself. The plain `/login` rather than the tenant-aware one, because the tenant-routes module excludes that path and `/t/<id>/login` answers 404
- The rest of the interface is translated, worked through in one pass with a scanner that reads text nodes (including ones mixed with `{{ }}`), attributes and script literals alike. Prices and their units, the bookable type badges, the mobile-key list badges and help panels, the account settings, the date and time inputs with their validation messages, cancellation terms, ticket dialogs, result strips and cards, map tooltips, empty states and the checkout's booking type names. New key groups `price.*` and `bookableType.*`; the remaining 40 keys that had no English — mobile-key stages and evidence, checkout, group bookings — are translated too, so every one of the 853 keys the app uses now resolves in both languages
- Dates, times and prices follow the language. `useFormatting` pinned every format to `de-DE`, so an English page still showed German dates and separators; the locale now comes from i18n (`de-DE` / `en-GB`, chosen so English keeps day-first ordering and a 24-hour clock). Month and weekday names come from `Intl` through the same composable instead of four hand-kept German arrays — the German output is unchanged, English comes for free
- Booking details are translated: the payment method (invoice, cash, bank transfer, credit and debit card, SOFORT and „not specified“ — the brand names stay as they are), the attachment types (invoice, receipt, user agreement, privacy policy, user manual, safety and product information), the failed-download notice and the „Your comment“ heading. Keys under `booking.payment.methods.*`, `booking.attachmentTypes.*`, `booking.download.*` and `booking.comment`, plus the card's own labels — booking number, booked on, status, booking period, payment information, total, payment method, „Active“, „Event time“, „Period“, „Price:“ and the „Download appointment“ action, which also appears on the detail page's more-actions menu next to „Share“ and the copied-link notification. the per-item card beside it (go to the bookable item, go to the event, unit price, quantity), „Booked on:“ and the attachment card in full — „Created:“, „Accepted:“, the unknown-bookable fallback and the line naming the document's type and its item, which is one key filled through `<i18n-t>` so the item keeps its italics and the word order stays the translator's to choose. All 91 keys the booking area uses resolve in both languages
- A second i18n pass covers what the first one missed, because it only read `<template>` blocks and these strings sit in `<script>`: the search bar's type options, address and period fields and the address hint, the navigation bar's sign-in and register buttons, the start page's category cards, the sort menus of the catalogue and the booking list, the booking and key filters (active, upcoming, past, all) with their status and payment options, the mobile-key help panel and its list and floor-plan toggle, the whole settings area, and the error page. Sign-in and registration are fully translated into English: the pages and cards themselves already read every string from a key, so the work was in `en.json`, which had none of those groups — `login`, `register`, `forgotPassword`, `resetPassword`, `emailVerification`, `sso`, plus the toasts in `notifications` and the card sign-in and card linking flows (`cardLogin`, `cardLink`, `cardLinkVerification`). All 141 keys the auth area uses now resolve in both languages
- The language button switches the language. It calls `setLocale`, which writes the `i18n_redirected` cookie the browser-language detection reads and re-renders the page in place. The console-log stub is gone
- About 100 hard-coded German strings across 40 components and pages now come from `de.json` / `en.json`: the filter panel and its category names, the search bar, result lists and map tooltips, the event and ticket blocks, the date and time inputs, the account, settings, invoice and favourites pages, the mobile-key pages and the error page. New keys sit under `common.*`, `filter.*`, `events.*`, `timePeriods.*`, `account.*`, `mobileKey.*`, `results.*`, `catalog.*` and `navigation.*`; `en.json` gained the `events` and `navigation` groups it never had. The prototype pages under `app/pages/prototype/` and `app/components/prototype/` are left as they are
- The filter area has a provider facet, under the categories: the tenants the current results come from, named from the tenant store, counted and sorted like the city facet, with a tenant the store has not loaded falling back to its id. It is labelled „Anbieter“ / „Provider“ (`filter.provider`), not „Mandant“, and it hides below two entries — inside a single tenant's catalogue every result carries the same id, so there is nothing to choose. The selection travels in the URL as `tenants=<id>,<id>`, counts towards „Filter zurücksetzen“ and towards the dot on the „Filtern“ button, and is cleared with the rest. The English locale gained the `filter` group, which it was missing altogether
- Mobile Key, Cooldown: after every sent command (open or close, also „Erneut öffnen“) the Control Button waits 8 s — grey, halo off, a thin ring draining clockwise, the remaining seconds as a large digit and the caption „Öffnen in {n} s möglich“ / „Abschließen in {n} s möglich“. A short tap during the Cooldown sends nothing and shakes the button once. Holding the button for 800 ms (touch, mouse, Space/Enter) reads the lock status without sending a command, with a ripple and refresh icon while holding and a spinner until the read returns; a hold during a running read is absorbed. No explanatory text; the `aria-label` names the gesture. Copy under `mobileKey.cooldown.*`
- Mobile Key, key row: doors in mode `authorization` or `code` show „Code an der Tür eingeben“ (bilingual); the „Per Knopf“ mode badge is gone from the key row and the key card. `needsCodeAtDoor()` replaces `ACCESS_POINT_MODES` / `accessPointMode`. The „Schlüssel“ section heading, the lock-state labels and the Access Window wordings live in `de.json` / `en.json`
- Mobile Key, Provider Support Contact: below the Control Button, in every stage except loading and evidence, one collapsed line „Problem an der Tür? Support: …“ shows the first present field (phone → email → name) as a `tel:` / `mailto:` link; a tap expands it to name, phone, e-mail and the booking number with a „bitte bereithalten“ hint. Error stages and the scan page's error block render it already expanded. `supportContactOf()` takes the Provider's contact as a whole when any field is filled and falls back to the tenant's general contact only when none is; the two are never mixed. `SupportContactLine` replaces `ProviderHelpSection`; the unused `EmergencyHelpModal` is gone. Copy under `mobileKey.supportContact.*`. A Provider that runs its own bookings has its own number named instead of ours: for an iFBS compartment the expanded line shows the „Vorgangsnummer“ (the grant's `authorizationId`, sent as `externalBookingId`) in place of the booking number (`supportReferenceOf`)
- Mobile Key, Confirmation Burst: after a command's answer the sheet shows the commanded state optimistically and confirms it with up to three status reads at about 1.5 s, 4 s and 6.5 s, stopping at the first match (open = `open === true`, close = `locked === true`); a close answer that already reports `locked` starts no burst. The lock's reading outranks the command: if the last readable status contradicts it the button flips without a new screen, and if nothing was readable the command's word stands. The immediate read after the command and the single settle read are gone; every burst read is one status audit row. On close the sheet hands its last known status to the key row, which does not re-read
- Mobile Key, Lock Busy: when the backend answers HTTP 423 or `code: "lock_busy"` to an open or close, the booker sees no error screen and no toast. The button keeps its stage, the Cooldown restarts from full and an amber line says „Das Schloss war noch beschäftigt. Die Wartezeit beginnt von vorn.“ (`mobileKey.cooldown.busy`). About 4 s later one plain status read is applied whatever it says; a failed read is ignored, a hold before then replaces it. The BFF open, close and open-status routes now pass the backend error body through as `data`, and the client attaches it to the thrown error next to `statusCode`
- Mobile Key, Access Window: each key row states its door's window in one of three wordings — „Zugang ab {from}“, „Zugang möglich bis {to}“, „Zugang endete um {to}“ (time only on the same day, the full „von … bis …“ as tooltip). The card badge (Kommend / Aktiv / Vergangen) and the list order come from the booking envelope `accessEligibility.accessWindow` the backend now sends (active, then upcoming ascending, then past descending); the hard-coded 60-minute constant is gone. `useAccessClock()` arms one timeout on the next boundary and re-evaluates on `visibilitychange` / `focus`: a key reaching its window end greys its button and flips badge and line without a request, a key reaching its window start reloads the list silently, and a return to the tab reloads only after a crossed boundary. `remoteOperable()` also requires `now` inside the door's own window; the open sheet turns to `too_late` after the window end, but never during a running Cooldown, burst or busy read. The `too_early` / `too_late` screens read the door's `accessFrom` / `accessTo` (bilingual)
- Booking details: the „Schlüssel“ section behaves like the Mobile Key. The per-booking points route now carries the backend's `accessEligibility` beside the points (backend 4.3, `readAccessPointsAnswer()` tolerates an older bare array by greying every button), the section attaches it to a local copy of the booking, so the Control Button is operable inside the Access Window, the window line and code hint show, and `useAccessClock()` reloads the points silently at a window start. A grey button stays silent; cancelled or rejected bookings keep the section with a grey button
- The footer names the instance's rights holder: `© <year> <copyright>` from the public instance's new `copyright` field (backend 4.3), rendered as text next to the legal-document links; with an empty or missing field it stays `© <year>` as before. No extra request — the field travels with the legal documents the footer already loads
...
- Blocks style themselves (the schema v1 amendment): a Panel behind a Block — its colour at its opacity, a corner radius and a 16 px frosting of what lies behind it, omitted where an opaque fill would hide it — a per-Block Offset of −3 to 3 rem on the 0.5 grid that moves that Block and not its stack neighbours (`x` from `md` up, `y` on every viewport, applied as authored in the Compact Hero), a Layer that lifts a Block in front of the Blocks it overlaps in every Zone, `align` on every Block type rather than on text alone, and a size and a colour per run of words in rich text, over a rich-text Block that now renders at its own step of the text scale. The middle-row Zone anchors centre their stack without a transform, which is what keeps a `front` Block from being trapped in its own Zone; a Panel is mode-independent, so `default` copy on one is black in both colour modes. The one behaviour change the admin sees: an overlap of a pair where either Block is offset or in front is no longer reported — both fields exist to make overlap deliberate — while the two edge warnings are never suppressed. **The storefront ships this amendment first, or together with the admin**: an older storefront ignores the new fields silently, with no error and only a preview that does not move, and the admin cannot detect that from the protocol

### Fixed

- Option lists that call `t()` follow the language switch. A plain `const items = [{ label: t(...) }]` is evaluated once when the component is created, so it keeps whichever language was active then — the booking and key period filters (all, upcoming, past) kept showing English after a visitor had switched there and back, the two verification failure cards kept their first language, the search bar's „Was suchen Sie?“ select kept „Buchungsobjekte“ / „Veranstaltungen“ after a switch to English, the result view toggle kept „Listenansicht“ / „Kartenansicht“, and the appearance settings kept their language names, which are themselves translated. They are `computed` now. Two pages built their labels that way and, worse, read `t` above the `const` that declares it, which throws before the page can render: the error page, and the checkout, whose booking type names (`TYPE_LABELS`) took the whole page down with a 500 — „Cannot access 't' before initialization“. In both, `t` moved to the top of the script and the labels became a `computed`. The checkout writes its type name into the shared `checkoutNavTab`, so that watcher now also watches the locale, the way the status page already did
- The bookable detail page opens again. `useFormatting` lives in `app/composables/utils/`, which Nuxt does not auto-import — it scans the top level of `composables/` and the `index` files of its subdirectories, nothing deeper — so four components that called it without an import threw „useFormatting is not defined“ during setup and answered 500: the detail page's price information, and the checkout's time-slot, week and month selections. They import it the way the other twelve callers already did
- The language select in the appearance settings works instead of sitting greyed out behind a „not available“ tooltip. It reads and writes the locale through `setLocale`, the same call the button in the navigation bar makes, and its entries come from the locales the app is configured with — German and English — rather than a hard-coded four, so it can no longer offer a language there is no file for. A locale without an `account.languages.*` key falls back to the name it carries in the config
- The sort button says „Sortieren: …“ / „Sort by: …“ instead of a German „Sortieren: “ glued in front of a translated mode. The prefix is one key with the mode as a placeholder (`filter.sort.button`), so the word order and the colon stay the translator's to choose
- The side navigation of the account pages follows the language too. Its entries live in `navigationPresets`, a module-level constant where `t()` is not available and a translated string would be frozen at import time, so they now carry a `labelKey` and `SideNavigation` translates where it renders; its „Coming soon“ tooltip came along. The quick-access card for invoices had its title as a German literal while its description already used a key
- The user menu and the bookings button follow the language switch. The menu's entries — Activity, Bookings, Digital keys, Invoices, Account — were German string literals, and `navigation.accountBookings` carried the German text as its English value, so the button read the same in both languages. The `navigation` group is now translated in full, which also fixes the menu's admin and settings entries
- Another shadowed translation group, the same way `account` was: `errors` appeared twice in `de.json`, so `errors.noCatalog` and `errors.noTenantFound` were silently dropped and showed as raw key paths. The blocks are merged
- Three translations the code asked for but no locale had, so visitors saw the raw key path: the reason shown on a rejected or cancelled booking (`account.bookingDetails.*`) was defined but then silently discarded, because `de.json` and `en.json` each carried the key `account` twice and JSON keeps only the last one; the failed-sign-out notification (`login.logoutErrorMessage.*`) and the heading for an already-confirmed card (`cardLinkVerification.error.alreadyVerifiedHeading`) had no entry at all
- Mobile Key: the Provider Support Contact showed the tenant's general contact although the Provider had one. The catalog bundle fills the tenant store with the catalog's reduced projection (no `accessApps`) and marks it initialized, so the full public list was never loaded; the store now fetches the access apps once and merges them in by tenant id without replacing the list (`fetchAccessApps`, `withAccessApps`). This also fixes the locker emergency-help accordion, which read the same store
- Maps show the map again instead of a grid of "Access blocked" tiles: nuxt-security defaults to `Referrer-Policy: no-referrer`, and OpenStreetMap answers a tile request that carries no Referer with a 403 placeholder tile. The storefront now sends `strict-origin-when-cross-origin` — the browser's own default, which leaks the bare origin and nothing more — so the tile servers can identify the app as their usage policy asks. Affects both map surfaces, the search result map and the address map on detail pages
- The booking list on `/account/bookings` no longer vanishes when one of the referenced events cannot be loaded: a 404 from the event endpoint (deleted event, or an event missing from the connected backend) used to reject the whole enrichment step, so the page rendered the empty state instead of the list. The event lookup now lives in `utils/bookingEventTimes.js`, falls back to the booking without event times per event, and requests each event once even when several bookings share it
- The booking list on `/account/bookings` no longer breaks apart on a full page load: the page derives its list from the async data instead of a `ref` copy of the store array that watchers never updated during SSR, so server and client now render the same branch and Vue no longer hydrates the card grid into a stale empty-state/skeleton wrapper
- No more stray gap between the navigation bar and the content on short panel pages (Mobile Key, invoices): `.container` centres horizontally only (`margin-inline: auto`); its former `margin: auto` also absorbed the free vertical space of the `flex-col` layouts. Bookable and event detail pages are no longer vertically centred on tall viewports for the same reason
...
- Both languages share one URL: the i18n strategy moves from `prefix_except_default` to `no_prefix`, so English no longer lives under `/en`. The app is not built for a prefix — about sixty links, redirects and middleware guards assemble raw paths (`/account`, `/login?redirect=…`), and `useTenantRoute().isActivePath` marks the active tab by comparing `route.path` against one, so under `/en` the checkout's redirects left the language and no tab ever looked active. The prefix only became reachable when the language button started working; the strategy had been configured that way all along. `customRoutes: "page"` goes with it (no page ever used it), as does the dead `/en/preview/**` pattern. The cost is that a language can no longer be linked to or indexed on its own — bringing the prefix back means first routing every link through a locale-aware helper
...
- Rich-text Blocks render in the Hero: the server-sanitised markup goes into the page as it arrives from the Theme View, at the Block's own step of the text scale — `xs` to `2xl`, `md` unless the Block says otherwise, one step down in the Compact Hero, with the Block's colour and shadow inherited into paragraphs, lists and links. Typography for the allowed tags is one stylesheet rule, `.hero-richtext`; a disallowed tag never reaches the page, and no sanitiser code is added to the client bundle. Hero links open in the same tab. The mobile Hero's grid column is `minmax(0, 1fr)`, so a Block with a fixed width wraps inside the phone instead of overflowing it
