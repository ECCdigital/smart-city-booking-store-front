...
- Tenant supervision, entry: a return target (`?redirect=`) survives login, card login, local signup, the verification mail (`nextUrl` to the backend, `?next=` on the mail link, `nextUrl` of the verify answer) and SSO including the `kc-redirect` / `kc-pending-redirect` cookies. One helper, `shared/utils/returnTarget.ts`, decides what may be followed: a relative in-app path or an address on the origin of `NUXT_PUBLIC_ADMIN_BASE_URL`; everything else falls back to `/` (the SSO routes redirected to an unchecked `redirect` before). It replaces `getSafeRedirectPath()`
- Backend `429` with `Retry-After` reaches the browser from the signup, resend-verification, verify-email and login proxies with status and header, and the pages say "try again in X seconds/minutes" (`rateLimit.*`, `shared/utils/retryAfter.ts`). The signup and resend proxies forward the visitor's address as `X-Forwarded-For`, so the backend's per-IP limits do not count the storefront server as one visitor
- Signup is account-neutral: the confirmation reads "if the address can be used you will receive a mail", a `409` lands on the same confirmation and the "e-mail already registered" text is gone. The confirmation offers to send the mail again (`/api/auth/resend-verification`, new)
- Footer entry "Angebote bereitstellen" / "Offer spaces" to the admin UI's `/onboarding`, only with `NUXT_PUBLIC_ADMIN_BASE_URL` set and a public instance that reports `allowAllUsersToCreateTenant: true`
- User menu: the Admin Entry ("Admin-Bereich" / "Admin area", new tab) for an instance owner or a user with an active membership, the Offer Spaces Entry ("Angebote bereitstellen" / "Offer spaces", same tab, to the admin UI's `/onboarding`) for a user who may create a tenant but has neither; decided by `adminEntry()` from the auth store's `permissions` (renamed from `permission`), so the menu no longer fetches `/api/memberships/my`. The menu's remaining labels are i18n keys (`navigation.*`)
...
- Tenant supervision, bookings of a tenant that is not public (pending approval or declined): the booking pages render from the booking answer (backend 4.x with the booking snapshot). Tenant name and support/emergency contact come from `booking.tenant` (`bookingTenantOf`), event title and times from `booking.event` where the catalog has none; the public tenant is only a fallback. The booking list tolerates a bundle 404, also under `/t/:tenantID/…`, and the status page no longer loads the public tenant. The tenant store persists only `currentTenantID` (`pick`; `paths` was ignored and the whole store landed in localStorage) and rewrites an older entry at start
- Tenant supervision, user menu: the English locale carries the `navigation` block ("Home", "Bookings", "Admin area", "Settings", …); the menu labels no longer fall back to German with English active
- Tenant supervision, errors pass through: the Nitro proxies on the catalog and booking path (catalog, tenants, bookables, events, checkout, coupon, payment apps, booking status) keep the backend's status code and pass its error body on as `data` through one helper (`proxyErrorOf`, `server/utils/proxyError.ts`); a backend that does not answer is a 502, never a 200 or an empty answer. `GET /api/checkout/:bookableID` and the checkout permission pre-check no longer swallow errors, `validate` no longer turns every error into a 400. In the bundle a detail the backend delivers for no tenant is a 404 for every catalog type, a failing backend stays an error (`detailResolution.ts`), and a list is an error instead of empty when no tenant answers (`listResolution.ts`). Bookable detail, event detail and checkout show one text for an offer that is missing, withdrawn or whose tenant is not public (pending approval or declined) — "Dieses Angebot ist nicht (mehr) verfügbar" (`errors.offerNotAvailable`) — and the detail pages answer HTTP 404 on SSR; the checkout tells a failed load apart from a missing offer. A booking attempt from a form opened before its tenant went non-public or the offer was withdrawn names it (`checkout.offer_not_reachable`). `error.vue` is bilingual with a neutral 404 text; the home page and the list stores keep a failed load apart from an empty list. Missing `en.json` counterparts added (`errors.noCatalog`, `errors.noTenantFound`, `events.noEvent`, `resources.noResource`)
- Tenant supervision, direct links: a bookable or event detail page loads its offer by id, independent of the catalog list. The BFF asks the backend per tenant — first the catalog interface (`/json/…`, listed offers), then the direct-link route (`/api/:tenant/bookables/public/:id`, `/api/:tenant/events/:id`) for an offer without a publication wish — and applies no rule of its own; what the backend does not deliver is not found (`detailResolution.ts`). `/t/:tenantID/…` detail requests no longer answer "Tenant Not Found" for a tenant without catalog participation; lists still do. Instance catalogs keep asking every listed tenant and additionally the tenant a link names (`?tenantId=`, now set by the booking-details links). The event detail page loads again on a cold direct link (the early `return` is gone). The detail page shows the name of an unlisted tenant without adding it to the tenant list; the empty-state back links keep the `/t/:tenantID` prefix
...
- Tenant supervision, no stale release from a cache: the two catalog bundle routes are no longer in the server-side SWR cache (`releaseSensitive`, whatever `NUXT_CACHE_ENABLED` says); `/api/catalog/mode` stays cached because it carries no tenant or offer. The proxy answers under `/api/catalog` (except `mode`), `/api/tenants`, `/api/bookables`, `/api/events` and `/api/checkout` go out with `Cache-Control: no-store`. On the client a new entry to a detail page fetches the detail again instead of reusing the store, drops a detail the backend no longer delivers, and the bundle's SSR payload answers the hydrating page only. Rollout: restart all storefront processes and purge CDN/proxy caches once — see "Tenant supervision rollout" in the README
