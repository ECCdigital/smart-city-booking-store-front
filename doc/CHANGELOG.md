...
- Tenant supervision, bookings of a tenant that is not public (pending approval or declined): the booking pages render from the booking answer. Tenant name and Provider Support Contact come from `booking.tenant`, event title and times from `booking.event` where the catalog has none; the booking list tolerates a bundle 404. The tenant store persists only `currentTenantID` (`paths` was ignored, the whole store landed in localStorage) and replaces an older entry at start
