...
- The Hero is admin-editable: the storefront renders the Hero Layout (schema v1) and the Background from the Theme Bundle, both the desktop and the mobile tree server-rendered at a reserved height, and serves the admin's Live Preview at `/preview/hero`. The hero data route `/api/theme/hero` and `/api/theme/logo` are gone with it. New environment variables: `NUXT_THEME_REVALIDATE_SECONDS` and `NUXT_THEME_REVALIDATE_TIMEOUT_MS` for the Theme Bundle's freshness; `NUXT_PUBLIC_ADMIN_BASE_URL` now also gates the Live Preview. The entries below hold the details; the Lighthouse acceptance run (Hero layout shift and largest paint, before and after) is documented in the pull request
