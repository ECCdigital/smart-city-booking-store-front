...
const bookablesID = computed(() => route.params.bookableId);

const {loadBundle} = useCatalogBundle()

const bookable = computed(() => {
  return bookableStore.getBookablesById(bookableID.value);
})
if(!bookable.value){
  await loadBundle({slug: catalogSlug.value, bookableID:bookablesID.value})
}
</script>
