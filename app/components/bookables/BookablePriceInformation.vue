...
const relevantPriceCategories = computed(() => {
  ...
  return props.item.priceCategories.filter(
    (category) => category.holidays.length === 0
  );
});
...
const forFreeHint = computed(() => {
  if (
    ...
    (props.item.priceCategories[0].priceEur === 0 ||
      !props.item.priceCategories[0].priceEur)
  ) {
...
const holidayHint = computed(() => {
  if (
    props.item.priceCategories.some((category) => category.holidays.length > 0)
  ) {
